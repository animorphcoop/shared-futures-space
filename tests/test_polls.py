...
from poll.models import MultipleChoicePoll, MultipleVote, SingleChoicePoll, SingleVote
...
def test_create_poll(client, test_user, test_river):
    ...
    # single choice
    new_poll = client.post(
        reverse("poll_create"),
        {
            "question": "is this a test poll?",
            "description": "a test poll",
            "kind": "SINGLE",
            "options": '["answer 1","answer b","all of the above"]',
            "expires": "01/02/2023 16:57",
            "river": str(test_river.id),
        },
    )
    ...
    assert "answer 1" in new_poll_redirect.content.decode("utf-8")
    assert MultipleChoicePoll.objects.filter(
        question="which of the following?"
    ).exists()
    assert (
        MultipleChoicePoll.objects.get(question="which of the following?").created_by
        == test_user
    )


def test_vote_poll_single(
    client, test_user, other_test_user, test_singlechoicepoll, test_river
):
    get_system_user()
    ...
    SingleVote.objects.create(
        poll=test_singlechoicepoll, user=other_test_user, choice=None
    )
    ...
    client.post(
        reverse("poll_view", args=[test_singlechoicepoll.uuid]),
        {"choice": test_singlechoicepoll.options[0]},
    )
    ...
    client.post(
        reverse("poll_view", args=[test_singlechoicepoll.uuid]),
        {"choice": test_singlechoicepoll.options[1]},
    )
    assert (
        len(
            SingleVote.objects.filter(
                poll=test_singlechoicepoll, user=test_user, choice=2
            )
        )
        == 0
    )
...
def test_vote_poll_multiple(
    client, test_user, other_test_user, test_multiplechoicepoll, test_river
):
    get_system_user()
    test_river.start_envision()
    client.force_login(test_user)
    RiverMembership.objects.create(
        river=test_river, user=test_user
    )  # so we can see the chat with the poll in
    RiverMembership.objects.create(river=test_river, user=other_test_user)
    MultipleVote.objects.create(poll=test_multiplechoicepoll, user=test_user, choice=[])
    MultipleVote.objects.create(
        poll=test_multiplechoicepoll, user=other_test_user, choice=[]
    )
    ...
    # can vote on the poll, and it won't close
    client.post(
        reverse("poll_view", args=[test_multiplechoicepoll.uuid]),
        {"choice": test_multiplechoicepoll.options[0]},
    )
    assert (
        len(
            MultipleVote.objects.filter(
                poll=test_multiplechoicepoll, choice__contains=[1]
            )
        )
        == 1
    )
    assert (
        len(
            MultipleVote.objects.filter(
                poll=test_multiplechoicepoll, choice__contains=[2]
            )
        )
        == 0
    )
    ...
    client.post(
        reverse("poll_view", args=[test_multiplechoicepoll.uuid]),
        {"choice": test_multiplechoicepoll.options[1]},
    )
    assert (
        len(
            MultipleVote.objects.filter(
    ...
    assert (
        len(
            MultipleVote.objects.filter(
                poll=test_multiplechoicepoll, user=test_user, choice__contains=[2]
            )
        )
        == 0
    )
    ...
    assert (
        MultipleChoicePoll.objects.get(id=test_multiplechoicepoll.id).check_closed()
        == True
    )
