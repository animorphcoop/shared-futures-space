...
def test_access_public(url, is_accessible, client):
    page = client.get(url)
    if is_accessible:
        assert page.status_code == 200 and "Page not found" not in page.content.decode(
            "utf-8"
        )
    else:
        assert page.status_code != 200 or "Page not found" in client.get(
            url
        ).content.decode("utf-8")


@pytest.mark.django_db
@pytest.mark.parametrize("url,is_accessible", [("/", True), ("/doesnotexist/", False)])
...
def test_access_admin_stuff(client, admin_client, test_user):
    ...
    assert "You are not an admin" not in str(
        admin_client.get("/profile/managerequests/").content
    )
