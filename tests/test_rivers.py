...
def test_river_membership(client, test_user, other_test_user, test_river):
    ...
    other_ownership = RiverMembership(
        user=other_test_user, river=test_river, starter=True
    )
    ownership.save()
    other_ownership.save()
    river_page_starter = client.get(reverse("view_river", args=[test_river.slug]))
    river_page_starter_html = bs4.BeautifulSoup(
        river_page_starter.content, features="html5lib"
    )
...
def test_river_management(client, test_user, other_test_user, test_river):
    ...
    other_membership = RiverMembership(
        user=other_test_user, river=test_river, starter=False
    )
    membership.save()
    other_membership.save()
    client.force_login(test_user)
    management_page = client.get(reverse("manage_river", args=[test_river.slug]))
    management_page_html = bs4.BeautifulSoup(
        management_page.content, features="html5lib"
    )
