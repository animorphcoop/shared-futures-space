...
def test_invoke_action(client, test_user, other_test_user, test_river):
    ...
    act = Action.objects.create(creator=other_test_user, receiver=test_user, kind='become_owner', param_river=test_river)
    RiverMembership.objects.create(user=test_user, river=test_river, owner=False)
    assert Action.objects.get(id=act.id).result == None
    client.post(reverse('do_action'), {'action_id': act.uuid, 'choice': 'invoke'})
    assert RiverMembership.objects.get(user=test_user, river=test_river).owner == True
...
def test_rescind_action(client, test_user, other_test_user, test_river):
    client.force_login(test_user)
    act = Action.objects.create(creator=test_user, receiver=other_test_user, kind='become_owner', param_river=test_river)
    RiverMembership.objects.create(user=test_user, river=test_river, owner=False)
    assert Action.objects.get(id=act.id).result == None
