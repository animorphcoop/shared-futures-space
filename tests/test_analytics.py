...
def test_log_account_create(client):
    previous_events = AnalyticsEvent.objects.filter(
        type=AnalyticsEvent.EventType.SIGNUP
    )
    assert len(previous_events) == 0
    exclude_list = [
        evt.id for evt in previous_events
...
def test_log_login(client, test_user):
    ...
    client.post(
        "/profile/login/", {"login": test_user.email, "password": "test_password"}
    )  # user passwords obviously not accessible as part of the object
    new_events = AnalyticsEvent.objects.filter(
        type=AnalyticsEvent.EventType.LOGIN
    ).exclude(id__in=exclude_list)
    assert len(new_events) == 1
    assert new_events[0].session.sessid_hash == make_password(
        test_user.display_name, salt=str(date.today())
    )
...
def test_log_resource_access(client, test_user, other_test_user, test_how_to_resource):
    ...
    assert new_events[0].target_resource.title == test_how_to_resource.title
    assert new_events[0].session.sessid_hash == make_password(
        other_test_user.display_name, salt=str(date.today())
    )
