...
class AnalyticsView(TemplateView):
    def get_context_data(self, **kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ctx["graphs"].append(
            BarGraph(
                "river/stages - overall",
                [
                    ("not begun", len(River.objects.filter(current_stage=None))),
                    (
                        "envision",
                        len(River.objects.filter(current_stage=River.Stage.ENVISION)),
                    ),
                    ("plan", len(River.objects.filter(current_stage=River.Stage.PLAN))),
                    ("act", len(River.objects.filter(current_stage=River.Stage.ACT))),
                    (
                        "reflect",
                        len(River.objects.filter(current_stage=River.Stage.REFLECT)),
                    ),
                ],
                river_scale,
            )
        ...
        for area in Area.objects.all():
            ctx["graphs"].append(
                BarGraph(
                    "river/stages - " + area.name,
                    [
                        (
                            "not begun",
                            len(River.objects.filter(area=area, current_stage=None)),
                        ),
                        (
                            "envision",
                            len(
                                River.objects.filter(
                                    area=area, current_stage=River.Stage.ENVISION
                                )
                            ),
                        ),
                        (
                            ...
                            len(
                                River.objects.filter(
                                    area=area, current_stage=River.Stage.PLAN
                                )
                            ),
                        ),
                        (
                            ...
                            len(
                                River.objects.filter(
                                    area=area, current_stage=River.Stage.ACT
                                )
                            ),
                        ),
                        (
                            ...
                            len(
                                River.objects.filter(
                                    area=area, current_stage=River.Stage.REFLECT
                                )
                            ),
                        ),
                    ],
                ...
                (
                    len(
                        RiverMembership.objects.filter(
                            river__in=River.objects.filter(area=area)
                        )
                    )
                    / len(River.objects.filter(area=area))
                )
                if len(River.objects.filter(area=area)) != 0
                else 0,
            )
            for area in Area.objects.all()
        ]
        ...
        ctx["graphs"].append(
            BarGraph("average swimmers/river", swimmers_area, swimmers_scale)
        )
