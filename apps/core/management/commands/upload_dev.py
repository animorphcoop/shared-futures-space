...
def add_rivers(rivers_data):
    for river_data in rivers_data:
        try:
            new_river = River.objects.get_or_create(
                title=river_data["name"],
                description=river_data["description"],
                area=Area.objects.get_or_create(name=river_data["area"])[0],
            )[0]
            ...
            for member in river_data["swimmers"]:
                RiverMembership.objects.get_or_create(
                    river=new_river, user=CustomUser.objects.get(display_name=member)
                )[0]
            for member in river_data["starters"]:
                m = RiverMembership.objects.get_or_create(
                    river=new_river, user=CustomUser.objects.get(display_name=member)
                )[0]
                m.starter = True
                m.save()
            if "envision" in river_data:
                ...
                for message in river_data["envision"]["chat"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.envision_stage.general_chat,
                    )
                    ...
                    for option in ["yes", "no"]:
                        for user in river_data["envision"]["poll"][option]:
                            SingleVote.objects.filter(
                                ...
                                poll=new_river.envision_stage.general_poll,
                            ).update(
                                choice=new_river.envision_stage.general_poll.options.index(
                                    option
                                )
                                + 1
                            )
            ...
            if "plan" in river_data:
                if new_river.envision_stage.general_poll is None:
                    new_river.envision_stage.general_poll = (
                        SingleChoicePoll.objects.create(
                            question="is this an acceptable vision?",
                ...
                for message in river_data["plan"]["general"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.plan_stage.general_chat,
                    )
                for message in river_data["plan"]["money"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.plan_stage.money_chat,
                    )
                ...
                for message in river_data["plan"]["time"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.plan_stage.time_chat,
                    )
            if "act" in river_data:
                ...
                for message in river_data["act"]["general"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.act_stage.general_chat,
                    )
                for message in river_data["act"]["money"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.act_stage.money_chat,
                    )
                for message in river_data["act"]["place"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.act_stage.place_chat,
                    )
                for message in river_data["act"]["time"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.act_stage.time_chat,
                    )
            if "reflect" in river_data:
                ...
                for message in river_data["reflect"]["chat"]:
                    Message.objects.get_or_create(
                        sender=CustomUser.objects.get(display_name=message["from"]),
                        text=message["content"],
                        chat=new_river.reflect_stage.general_chat,
                    )

        except Exception as e:
            print(
                "could not load river image: "
                + str(river_data["name"])
                + "\nerror given: "
                + repr(e)
            )
...
def add_relations(relations_data):
    for userchat_data in relations_data["User Chat"]:
        try:
            pair = get_userpair(
                CustomUser.objects.get(display_name=userchat_data["user1"]),
            ...
            print(
                "could not add userchat with definition: "
                + str(userchat_data)
                + "\nerror given: "
                + repr(e)
            )
