...
class TaskListView(TaskViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        ...
        tasks = river.task_set.filter(stage_name=self.kwargs["stage"]).order_by(F("due").asc(nulls_last=True))
        context.update(dict(
            river=river,
            tasks=tasks,
        ))
...
class TaskCreateView(TaskViewMixin, CreateView):
    ...
    def get_success_url(self) -> str:
        ...
        if self.request.htmx:
            return reverse('river_task_list', args=[self.get_slug(), self.get_stage_name(), self.get_topic()])
        return reverse('river_chat', args=[self.get_slug(), self.get_stage_name(), self.get_topic()])
