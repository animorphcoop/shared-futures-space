...
class PollView(TemplateView):
    def post(self, request: WSGIRequest, uuid: UUID) -> HttpResponseRedirect:
        ...
        if request.user.is_active and not poll.check_closed() and 'choice' in request.POST and Vote.objects.filter(user = request.user, poll = poll).exists():
            try:
                choice = poll.options.index(request.POST['choice']) + 1
            except ValueError:
                choice = 0
            Vote.objects.filter(poll = poll, user = request.user).update(choice = choice)
            poll.check_closed()
...
class PollCreateView(CreateView): # pyre-ignore[24]
    ...
    def form_valid(self, form) -> HttpResponseRedirect: # pyre-ignore[2] - the type of the form argument is some weird private thing that i can't seem to get hold of
        new_poll = Poll.objects.create(question = form.instance.question, options = form.instance.options, expires = form.instance.expires)
