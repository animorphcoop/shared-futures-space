...
def validate_poll_options(value):
...
class Vote(models.Model):
    ...
    def clean(self):
        ...
        if self.choice < 0 or self.choice > len(self.poll.options):
            raise ValidationError('not a valid choice for that poll (got ' + str(self.choice) + ', expected an integer in 0 - ' + len(self.poll.options) + ')')
