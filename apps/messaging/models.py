...
class Message(models.Model):
    ...
    sender: models.ForeignKey = models.ForeignKey(
        "userauth.CustomUser", null=True, on_delete=models.SET_NULL
    )
    text: models.CharField = models.CharField(max_length=2000, blank=True)
    image: models.ImageField = models.ImageField(
        upload_to="messages/images/", blank=True
    )
    ...
    reply_to: models.ForeignKey = models.ForeignKey(
        "messaging.Message", null=True, on_delete=models.SET_NULL
    )
    chat: models.ForeignKey = models.ForeignKey(
        "messaging.Chat", on_delete=models.CASCADE
    )
    hidden: models.BooleanField = models.BooleanField(default=False)
    hidden_reason: models.CharField = models.CharField(
        max_length=25, null=True, default=None
    )
    # context_* values are nullable fields used to fill in the gaps in system messages
    context_action: models.ForeignKey = models.ForeignKey(
        "action.Action", null=True, on_delete=models.SET_NULL
    )
    context_river: models.ForeignKey = models.ForeignKey(
        "river.River", null=True, on_delete=models.SET_NULL
    )
    context_user_a: models.ForeignKey = models.ForeignKey(
        "userauth.CustomUser",
        null=True,
        on_delete=models.SET_NULL,
        related_name="user_a",
    )
    context_user_b: models.ForeignKey = models.ForeignKey(
        "userauth.CustomUser",
        null=True,
        on_delete=models.SET_NULL,
        related_name="user_b",
    ...
    def text_with_links(self) -> str:
        ...
        for match in url_regex.finditer(text):
            ...
            text = (
                text[: start + chars_added] + link_in_html + text[end + chars_added :]
            )
