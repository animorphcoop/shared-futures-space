...
from apps.core.utils.tags_declusterer import (
    objects_tags_cluster_list_overwrite,
    single_object_tags_cluster_overwrite,
)

from .models import CaseStudy, HowTo, SavedResource
...
def resource_tag(request: HttpRequest, tag: str) -> HttpResponse:
    resources = retrieve_and_chain_resources()
    context = {"resources": resources, "tag": tag}
    return render(request, "resources/resources.html", context)
...
def resource_search(request: HttpRequest) -> HttpResponse:
    ...
    results = (
        filter_and_cluster_resources(search_text, order_by)
        if search_text != ""
        else retrieve_and_chain_resources()
    )
    ...
    if request.user.is_authenticated:
        ...
        for resource_card in results:
            try:
                saved_instance = SavedResource.objects.get(
                    saved_resource=resource_card, saved_by=current_user
                )
...
def filter_and_cluster_resources(
    ...
    if order_by == "latest":
        ...
        results.sort(
            key=lambda r: len(SavedResource.objects.filter(saved_resource=r)),
            reverse=True,
        )
        ...
        results.sort(
            key=lambda r: len(
                AnalyticsEvent.objects.filter(
                    type=AnalyticsEvent.EventType.RESOURCE, target_resource=r
                )
            ),
            reverse=True,
        )
...
def resource_saved(request: HttpRequest, res_id: Optional[int]) -> HttpResponse:
    ...
    context = {"resource_id": res_id, "status": saved_resource}
